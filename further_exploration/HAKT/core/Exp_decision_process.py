...
import numpy as np
import os
from HAKT.core.model import KTModel

from HAKT.core.vis_utils import VisTools
...
def reliability(loader, args, k):
    ...
    model = KTModel(args, device).to(device)
    ...
    path = '../dataset/%s/adj_mat' % args.data_set
    ...
    for seq_lens, pad_data, pad_answer, pad_index, pad_label in loader['test']:
        _ = model(pad_data, pad_answer, pad_index)  # run the model

        # [seq, batch, num_skill]
        seq_ques_skill = F.embedding(pad_data, ques_skill_mat)
        # [seq, batch, num_skill]
        seq_ques_skill = seq_ques_skill.transpose(0, 1)
        # [seq, batch, num_template]
        seq_ques_template = F.embedding(pad_data, ques_template_mat)
        # [seq, batch, num_template]
        seq_ques_template = seq_ques_template.transpose(0, 1)
        # [batch, seq, seq]
        seq_ques_relate_skill = torch.matmul(seq_ques_skill, seq_ques_skill.transpose(1, 2))
        # [batch, seq, seq]
        seq_ques_relate_template = torch.matmul(seq_ques_template, seq_ques_template.transpose(1, 2))
